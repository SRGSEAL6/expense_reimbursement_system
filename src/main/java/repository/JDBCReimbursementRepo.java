...
public class JDBCReimbursementRepo implements ReimbursementRepo {
    ...
    public Optional<Reimbursement> findByReimId(String reimId) {
        LOG.info("Loading employee: " + reimId);    //LOG for debugging
        try (Connection connection = SQLConnectionFactory.getConnection()) { //This will automatically close the connection
            String sqlCode = "select * from reimbursements where reim_id=?"; //SQL statement from postgres to get every column from employee table
            PreparedStatement ps = connection.prepareStatement(sqlCode); //Pre compiles the sqlcode
            ps.setInt(1, Integer.parseInt(reimId));
            ...
            LOG.error("Error loading employee: " + reimId);
            e.printStackTrace();
        }
